...
class Book {
  ...
  static removeBook(book) {
    ...
    if (index !== -1) {
      ...
      Book.displayBooks(); // Update the display
    }
  }
  ...
  static displayBooks(){
    ...
    for(let book of myLibrary){
      ...
      removeButton.addEventListener('click', () => {
        Book.removeBook(book); // Call a function to remove the book from the library and update the display
      });
...
Book.addBookToLibrary(theHobbit, theBible, sorcerersStone)
Book.displayBooks()
...
newBookForm.addEventListener("submit", function(event) {
  ...
  Book.addBookToLibrary(new Book(title, author, pages, read));
  Book.displayBooks();
